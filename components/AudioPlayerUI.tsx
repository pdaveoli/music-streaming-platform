...
function findCurrentLyricIndex(lyrics: { time: number; text: string }[], time: number): number | null {
...
export default function PersistentAudioPlayerUI() {
  ...
  const [lyricsCurrentIndex, setLyricsCurrentIndex] = useState<number | null | -1>(null); // Use -1 for before first lyric
  ...
  const toggleMute = () => {
    if (audioRef.current) {
      ...
      if (!newMutedState && volume === 0) { // If unmuting and volume was 0, set to a default
        setVolume(0.5);
  ...
  useEffect(() => {
    if (currentTrack && currentTrack.lyricsUrl) {
      fetch(currentTrack.lyricsUrl)
        .then((response) => {
            if (!response.ok) throw new Error(`Failed to fetch lyrics: ${response.statusText}`);
            return response.text();
        })
        .then((lrcContent) => {
          const parsed = parseLyrics(lrcContent);
          setSyncedLyrics(parsed);
          setLyricsCurrentIndex(findCurrentLyricIndex(parsed, currentTime ?? 0)); // Initialize index
        })
        .catch((error) => {
  ...
  useEffect(() => {
    if (showLyrics && activeLyricLineRef.current && lyricsScrollContainerRef.current) {
      ...
      <div className="w-full flex items-center justify-between h-[60px]">
        ...
        <div className="flex items-center gap-3 justify-center flex-grow min-w-0 px-4"> {/* Added px-4 for some breathing room */}
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-sm truncate" title={currentTrack.name}>{currentTrack.name}</h3>
            <p className="text-xs text-muted-foreground truncate" title={currentTrack.artist}>{currentTrack.artist}</p>
            <div className="w-full mt-1 group" onClick={handleSeek} style={{ cursor: "pointer" }}>
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 flex-shrink-0"> {/* Adjusted gap */}
            <Button variant="ghost" size="icon" onClick={playPrev} title="Previous"><RewindIcon className="w-5 h-5 sm:w-6 sm:h-6" /></Button>
            <Button variant="ghost" size="icon" onClick={togglePlayPause} title={isPlaying ? "Pause" : "Play"} className="w-8 h-8 sm:w-10 sm:h-10">
              {isPlaying ? <PauseIcon className="w-5 h-5 sm:w-6 sm:h-6" /> : <PlayIcon className="w-5 h-5 sm:w-6 sm:h-6" />}
            </Button>
            <Button variant="ghost" size="icon" onClick={playNext} title="Next"><FastForwardIcon className="w-5 h-5 sm:w-6 sm:h-6" /></Button>
          </div>
        </div>

        <div className="flex items-center gap-2 flex-shrink-0">
          <Button variant="ghost" size="icon" onClick={toggleMute} title={isMuted ? "Unmute" : "Mute"}>{getVolumeIcon()}</Button>
          <Input
          ...
          <div className="relative">
            <Button variant="ghost" size="icon" title="Lyrics" onClick={() => setShowLyrics(!showLyrics)} className={showLyrics ? "text-green-400" : ""}>
                  ...
                  {syncedLyrics.length > 0 ? (
                    syncedLyrics.map((lyric, index) => {
                      ...
                      if (isActive) {
                        ...
                        lineClasses += " text-base sm:text-lg text-gray-500 dark:text-gray-600 opacity-50"; // Past: dimmer, smaller
                      } else { // Future lines
                        lineClasses += " text-base sm:text-lg text-gray-400 dark:text-gray-500 opacity-70"; // Future: less dim
                      }
                    ...
                    <p className="text-gray-400 dark:text-gray-500 text-sm text-center py-10">
                      {currentTrack.lyricsUrl ? "Loading lyrics..." : "No lyrics available for this track."}
                    </p>
                  )}
                </div>
              </div>
