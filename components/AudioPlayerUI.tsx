...
export default function PersistentAudioPlayerUI() {
    ...
    const currentTrack = currentTrackIndex !== null && tracks[currentTrackIndex] ? tracks[currentTrackIndex] : null;
        ...
        return (
            <div className="fixed top-0 left-0 right-0 bg-background/80 backdrop-blur-md border-b p-3 shadow-lg h-[84px] z-50">
                <div className="max-w-5xl mx-auto flex items-center gap-3 h-full">
                    ...
                    <div className="w-9 h-9 bg-muted rounded-full animate-pulse"></div> {/* Placeholder for new URL input area */}
                </div>
            </div>
        );
    }
    ...
    if (!currentTrack) {
        return (
            ...
            <div className="max-w-5xl mx-auto flex items-center justify-between gap-3 h-[60px]">
                ...
                <p className="text-sm text-muted-foreground">No track selected. Press play or load another URL.</p>
                <div className="flex items-center gap-2">
                    <Button variant="ghost" size="icon" onClick={() => tracks.length > 0 && togglePlayPause()} title="Play first track" disabled={tracks.length === 0}>
                ...
                <div className="flex items-center gap-3 justify-center w-[600px] min-w-0"> 
                    ...
                    <div className="flex-1 min-w-0"> 
                        ...
                        <div className="w-full mt-1 group" onClick={handleSeek} style={{ cursor: 'pointer' }}>
                            <Progress value={progress} className="h-1 group-hover:h-1.5 transition-all duration-150 ease-in-out" />
                    ...
                    <div className="flex items-center gap-2">
                        <Button variant="ghost" size="icon" onClick={handleShuffle} title="Shuffle">
                            {isShuffling ? <ShuffleIcon className="w-5 h-5 text-blue-500" /> : <ShuffleIcon className="w-5 h-5" />}
                        </Button>
                        <Button variant="ghost" size="icon" onClick={handleRepeat} title="Repeat">
                            {isRepeating ? <Repeat1Icon className="w-5 h-5 text-blue-500" /> : <RepeatIcon className="w-5 h-5" />}
                        </Button>
                    </div>
                    {/* Main playback controls */}
                    <div className="flex items-center gap-1">
                        <Button variant="ghost" size="icon" onClick={playPrev} title="Previous">
                            <RewindIcon className="w-5 h-5" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={togglePlayPause} title={isPlaying ? "Pause" : "Play"} className="w-10 h-10">
